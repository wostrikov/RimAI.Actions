...
public static class RimAICapabilityMigrationRouter
{
    public static bool TryExecute(ExecutionContext context, out ExecutionResult result)
    {
        ...
        if (RestCapabilityCatalog.TryResolve(ownership.CapabilityId, out _))
        {
            result = Map(
                context,
                RimAIRestCapabilities.Execute(
                    context.ResolvedActor,
                    context.ResolvedTarget,
                    ownership.CapabilityId,
                    Metadata(context, ownership.CapabilityId))
        ...
        if (CommsCapabilityCatalog.TryResolve(ownership.CapabilityId, out _))
        {
            result = Map(
                context,
        ...
        if (CraftCapabilityCatalog.TryResolve(ownership.CapabilityId, out _))
        {
            result = Map(
                context,
                RimAICraftCapabilities.Execute(
                    context.ResolvedActor,
                    context.ResolvedTarget,
                    ownership.CapabilityId,
                    Metadata(context, ownership.CapabilityId))
